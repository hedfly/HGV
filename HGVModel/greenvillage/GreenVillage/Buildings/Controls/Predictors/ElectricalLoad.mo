within GreenVillage.Buildings.Controls.Predictors;
block ElectricalLoad "Block that predicts an electrical load"
  ...
  Modelica.Blocks.Interfaces.RealInput TOut(unit="K") if (predictionModel ==
  ...
  Buildings.Controls.Interfaces.DayTypeInput typeOfDay[integer((nPre - 1)/nSam)
     + 2] "Type of day for the current and the future days for which a prediction is to be made.
...
protected
  ...
  discrete output Modelica.SIunits.Power P[Buildings.Controls.Types.nDayTypes,
  ...
  discrete output Modelica.SIunits.Temperature T[if predictionModel == Types.PredictionModel.WeatherRegression
  ...
  Boolean historyComplete[Buildings.Controls.Types.nDayTypes,nSam](each start=
  ...
  discrete Modelica.SIunits.Power PPreHis[Buildings.Controls.Types.nDayTypes,
    nSam] "Predicted power consumptions for all day off time intervals";
  Boolean PPreHisSet[Buildings.Controls.Types.nDayTypes,nSam](each start=false,
      each fixed=true)
...
initial equation
  ...
  iHis = zeros(Buildings.Controls.Types.nDayTypes, nSam);
   ...
   _storeHistory = true;
  samStart = Buildings.Controls.Predictors.BaseClasses.sampleStart(t=time,
  ...
  when sampleTrigger then
    ...
    if predictionModel == Types.PredictionModel.Average then
      for m in 1:nPre loop
        ...
        // in future versions.
        PPre[m] := Buildings.Controls.Predictors.BaseClasses.average(P={P[
          _typeOfDay[m], iSam[m], i] for i in 1:nHis}, k=if historyComplete[
          _typeOfDay[m], iSam[m]] then nHis else iHis[_typeOfDay[m], iSam[m]]);
      end for;
    elseif predictionModel == Types.PredictionModel.WeatherRegression then
      for m in 1:nPre loop
        PPre[m] := Buildings.Controls.Predictors.BaseClasses.weatherRegression(
          ...
          k=if historyComplete[_typeOfDay[m], iSam[m]] then nHis else iHis[
            _typeOfDay[m], iSam[m]]);
      end for;
    else
      PPre:= zeros(nPre);
