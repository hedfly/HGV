...
model X3BWithRadiantFloor "Example model showing a use of X3B"
  ...
  Buildings.Fluid.Sources.Boundary_pT airOut(nPorts=1, redeclare package Medium
      = Air) "Air outlet for X3A"
  ...
  Buildings.Rooms.FLEXLAB.Rooms.X3B.Closet clo(
    redeclare package Medium = Air,
    nPorts=2,
    energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial)
    "Model of the closet"
  ...
  Buildings.Fluid.Sources.MassFlowSource_T airInEle(
    ...
    annotation (Placement(transformation(extent={{-10,-10},{10,10}}, origin={-6,
            -42})));
  ...
  Buildings.Fluid.Sources.MassFlowSource_T airInClo(
    ...
    use_T_in=true,
    redeclare package Medium = Air,
    nPorts=1) "Inlet air conditions (from AHU) for the closet" annotation (
      Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=180,
        origin={50,116})));
  Buildings.Fluid.Sources.Boundary_pT airOutClo(redeclare package Medium = Air,
      nPorts=1) "Air outlet from the closet" annotation (Placement(
        transformation(extent={{-10,-10},{10,10}}, origin={50,80})));
