within GreenVillage.Buildings.Fluid.SolarCollectors.BaseClasses;
...
equation
  ...
  iamBea = Buildings.Utilities.Math.Functions.smoothHeaviside(1/3*Modelica.Constants.pi
    ...
    B1);
  // E+ Equ (556)
  iam = Buildings.Utilities.Math.Functions.smoothHeaviside(1/3*Modelica.Constants.pi
     - incAng, Modelica.Constants.pi/60)*((HDirTil*iamBea + HSkyDifTil*iamSky
     + HGroDifTil*iamGro)/Buildings.Utilities.Math.Functions.smoothMax(
  ...
  for i in 1 : nSeg loop
    QSol_flow[i] = A_c/nSeg*(y_intercept*iam*(HDirTil*(1.0 - shaCoe_internal)
       + HSkyDifTil + HGroDifTil))*smooth(1, if TFlu[i] < TMedMax2 then 1 else
      Buildings.Utilities.Math.Functions.smoothHeaviside(TMedMax - TFlu[i],
      dTMax));
  end for;
