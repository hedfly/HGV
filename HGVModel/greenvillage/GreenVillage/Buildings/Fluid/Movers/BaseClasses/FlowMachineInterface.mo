within GreenVillage.Buildings.Fluid.Movers.BaseClasses;
partial model FlowMachineInterface
  ...
  extends Buildings.Fluid.Movers.BaseClasses.PowerInterface(
    ...
    _perPow(
      hydraulicEfficiency=_per_y.hydraulicEfficiency,
  ...
  import cha = GreenVillage.Buildings.Fluid.Movers.BaseClasses.Characteristics;
...
protected
  ...
  final parameter
    ...
    final n=nOri,
    final V_flow=if (haveVMax and haveDPMax) or (nOri == 2) then {_per_y.pressure.V_flow[
        i] for i in 1:nOri} else zeros(nOri),
    final dp=if (haveVMax and haveDPMax) or (nOri == 2) then {(_per_y.pressure.dp[
        i] + _per_y.pressure.V_flow[i]*kRes) for i in 1:nOri} else zeros(nOri))
  ...
  parameter
    ...
    V_flow=if (haveVMax and haveDPMax) or (nOri == 2) then zeros(nOri + 1)
        ...
        1,
        {0},
        {_per_y.pressure.V_flow[i] for i in 1:nOri}) elseif haveDPMax then cat(
        1,
...
initial algorithm
  ...
  assert(Buildings.Utilities.Math.Functions.isMonotonic(x=_per_y.pressure.V_flow,
    strict=true) and _per_y.pressure.V_flow[1] > -Modelica.Constants.eps, "The fan pressure rise must be a strictly decreasing sequence with respect to the volume flow rate,
