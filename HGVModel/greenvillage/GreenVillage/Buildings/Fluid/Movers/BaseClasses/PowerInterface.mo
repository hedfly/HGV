...
initial algorithm
 // Compute derivatives for cubic spline
  motDer := if _perPow.use_powerCharacteristic then zeros(size(_perPow.motorEfficiency.V_flow,
    1)) elseif (size(_perPow.motorEfficiency.V_flow, 1) == 1) then {0} else
    ...
    ensureMonotonicity=Buildings.Utilities.Math.Functions.isMonotonic(x=_perPow.motorEfficiency.eta,
      strict=false));
  hydDer := if _perPow.use_powerCharacteristic then zeros(size(_perPow.hydraulicEfficiency.V_flow,
    1)) elseif (size(_perPow.hydraulicEfficiency.V_flow, 1) == 1) then {0}
     else Buildings.Utilities.Math.Functions.splineDerivatives(x=_perPow.hydraulicEfficiency.V_flow,
