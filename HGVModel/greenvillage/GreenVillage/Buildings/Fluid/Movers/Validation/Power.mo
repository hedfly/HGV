within GreenVillage.Buildings.Fluid.Movers.Validation;
model Power "Power calculation comparison among three mover types"
  ...
  parameter
    Buildings.Fluid.Movers.BaseClasses.Characteristics.efficiencyParameters efficiency(V_flow=
        per.pressure.V_flow, eta=sqrt(per.pressure.V_flow .* per.pressure.dp
         ./ {Buildings.Fluid.Movers.BaseClasses.Characteristics.power(
        per=per.power,
        V_flow=i,
        r_N=1,
        delta=0.01,
        d=Buildings.Utilities.Math.Functions.splineDerivatives(x=per.power.V_flow,
          y=per.power.P)) for i in per.pressure.V_flow}))
  ...
  Buildings.Fluid.Sources.Boundary_pT sink(nPorts=3, redeclare package Medium
      = Medium)
