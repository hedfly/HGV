within GreenVillage.Buildings.Fluid.Actuators.Valves;
...
initial equation
  ...
  assert(Buildings.Utilities.Math.Functions.isMonotonic(x=flowCharacteristics.y,
    strict=true),
    "The values for y in flowCharacteristics must be strictly monotone increasing.");
  assert(Buildings.Utilities.Math.Functions.isMonotonic(x=flowCharacteristics.phi,
    strict=true),
