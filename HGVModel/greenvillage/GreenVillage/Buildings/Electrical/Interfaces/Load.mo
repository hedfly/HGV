within GreenVillage.Buildings.Electrical.Interfaces;
model Load "Partial model for a generic load"
  ...
  Modelica.Blocks.Interfaces.RealInput y(min=0, max=1, unit="1") if (mode ==
    Buildings.Electrical.Types.Load.VariableZ_y_input)
  ...
  replaceable Buildings.Electrical.Interfaces.Terminal terminal(redeclare
    ...
    "Generalized electric terminal" annotation (Placement(transformation(extent=
           {{-108,-8},{-92,8}}), iconTransformation(extent={{-108,-8},{-92,8}})));
...
equation
  ...
  if mode == Buildings.Electrical.Types.Load.FixedZ_steady_state or mode ==
      Buildings.Electrical.Types.Load.FixedZ_dynamic then
    y_internal   = 1;
