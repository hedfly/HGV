within GreenVillage.Buildings.Electrical.AC.OnePhase.Loads.Examples;
model ParallelLoads
  ...
  Buildings.Electrical.AC.OnePhase.Loads.Inductive varRL(
    ...
    P_nominal=-1e3,
    linearized=false,
    V_nominal=120) "Variable inductor and resistor" annotation (Placement(
        transformation(extent={{-10,-10},{10,10}}, origin={10,50})));
  Buildings.Electrical.AC.OnePhase.Sources.FixedVoltage source(f=60, V=120)
    "Voltage source" annotation (Placement(transformation(extent={{-10,-10},{10,
            10}}, origin={-70,10})));
  ...
  Buildings.Electrical.AC.OnePhase.Loads.Inductive RL(
    P_nominal=-1e3,
    linearized=false,
    V_nominal=120) "Constant inductor and resistor" annotation (Placement(
  ...
  Buildings.Electrical.AC.OnePhase.Loads.Resistive R(
    ...
    mode=Buildings.Electrical.Types.Load.FixedZ_steady_state,
    linearized=false,
    V_nominal=120) "Resistive load" annotation (Placement(transformation(extent=
           {{-10,-10},{10,10}}, origin={10,-30})));
