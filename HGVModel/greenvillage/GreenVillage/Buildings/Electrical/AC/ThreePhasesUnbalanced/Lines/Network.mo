within GreenVillage.Buildings.Electrical.AC.ThreePhasesUnbalanced.Lines;
...
equation
  ...
  for i in 1:grid.nNodes loop
    VAbs[1, i] = Buildings.Electrical.PhaseSystems.OnePhase.systemVoltage(
      terminal[i].phase[1].v);
    VAbs[2, i] = Buildings.Electrical.PhaseSystems.OnePhase.systemVoltage(
      terminal[i].phase[2].v);
    VAbs[3, i] = Buildings.Electrical.PhaseSystems.OnePhase.systemVoltage(
