...
partial model PrescribedFlowMachine
  ...
  extends Buildings.Obsolete.Fluid.Movers.BaseClasses.FlowMachineInterface(
      V_flow_max(start=V_flow_nominal), final rho_default=Medium.density_pTX(
        p=Medium.p_default,
        T=Medium.T_default,
        X=Medium.X_default));

  extends Buildings.Obsolete.Fluid.Movers.BaseClasses.PartialFlowMachine(final
      m_flow_nominal=max(pressure.V_flow)*rho_default, preSou(final
