...
model Heating_coil_control
 ...
 parameter Real Pos_min "The minimum valve position";
  LimPID                               conPID(controllerType=Modelica.Blocks.Types.SimpleController.PI,
    ...
    Ti=ti,
    yMin=Pos_min,
    reverseAction=false)                                                                                       annotation (Placement(transformation(extent={{-10,-10},{10,10}})));
  ...
  Modelica.Blocks.Sources.RealExpression realExpression(y=if On > 0 then
        u_m else u_s)
...
equation
  ...
  connect(conPID.u_s, u_s) annotation (Line(points={{-12,0},{-12,0},{-120,0}},
                          color={0,0,127},
      pattern=LinePattern.Dash));
  connect(realExpression.y, conPID.u_m) annotation (Line(
      ...
      color={0,0,127},
      pattern=LinePattern.Dash));
  connect(conPID.On, On) annotation (Line(
