...
model UserSide_withPump
 package Medium = Buildings.Media.Water "Medium model";
  ...
  Buildings.Fluid.MixingVolumes.MixingVolume rooVol(
    ...
    m_flow_nominal=m2_flow_nominal) "Volume of air in the room"
  ...
  Modelica.Blocks.Interfaces.RealInput Q annotation (Placement(transformation(extent={{-140,40},{-100,80}})));
  Modelica.Blocks.Interfaces.RealInput CoolOn
    "Actuator position (0: closed, 1: open)"                                           annotation (Placement(transformation(extent={{-140,-20},{-100,20}})));
  Modelica.Blocks.Interfaces.RealInput TSetPoi
    "Connector of setpoint input signal"                                            annotation (Placement(transformation(extent={{-140,-80},{-100,-40}})));
  Buildings.Fluid.Movers.FlowControlled_m_flow fan(redeclare package Medium =
        Medium, m_flow_nominal=m2_flow_nominal,
    motorCooledByFluid=false)                      annotation (Placement(transformation(
        extent={{-6,6},{6,-6}},
        rotation=90,
        origin={20,40})));
  ...
  Modelica.Blocks.Math.Gain gain(k=m2_flow_nominal)
  ...
  Buildings.Fluid.Movers.FlowControlled_dp     fan1(
    ...
    m_flow_nominal=m1_flow_nominal)                annotation (Placement(transformation(
        ...
        rotation=90,
        origin={-40,-78})));
  Modelica.Blocks.Math.Gain gain1(k=dp_nominal)
...
equation
  ...
  connect(QSou.Q_flow, Q) annotation (Line(
      ...
      smooth=Smooth.None,
      pattern=LinePattern.Dash));
  connect(rooVol.ports[1], heatPump.port_a2) annotation (Line(
      points={{-1,50},{-20,50},{-20,34},{-6,34},{-6,10}},
      color={0,127,255},
      smooth=Smooth.None,
      thickness=1));
  connect(heatPump.CoolOn, CoolOn) annotation (Line(
      points={{16,8},{-32,8},{-32,0},{-120,0}},
      color={0,0,127},
      smooth=Smooth.None,
      pattern=LinePattern.Dash));
  connect(heatPump.TSetPoi, TSetPoi) annotation (Line(
      points={{16,-8},{38,-8},{38,-60},{-120,-60}},
      color={0,0,127},
      smooth=Smooth.None,
      pattern=LinePattern.Dash));
  connect(heatPump.port_b2, fan.port_a) annotation (Line(
      points={{6,10},{6,34},{20,34}},
      color={0,127,255},
      smooth=Smooth.None,
      thickness=1));
  connect(fan.port_b, rooVol.ports[2]) annotation (Line(
      points={{20,46},{20,50},{3,50}},
      color={0,127,255},
      smooth=Smooth.None,
      thickness=1));
  ...
  connect(heatPump.On, On) annotation (Line(
      ...
      smooth=Smooth.None,
      pattern=LinePattern.Dash));
  connect(gain.y, fan.m_flow_in) annotation (Line(
      points={{35.6,40},{32,40},{32,39.88},{27.2,39.88}},
      color={0,0,127},
      smooth=Smooth.None,
      pattern=LinePattern.Dash));
