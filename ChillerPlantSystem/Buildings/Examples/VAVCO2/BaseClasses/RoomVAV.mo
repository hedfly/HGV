...
model RoomVAV "Model for CO2 emitted by people"
  ...
  replaceable model MotorModel = Buildings.Fluid.Actuators.Motors.IdealMotor (
        delta=0.02, tOpe=60);
  ...
  Buildings.Fluid.MixingVolumes.MixingVolume vol(
    ...
    annotation (extent=[-10,-10; 10,10], Placement(transformation(extent={{-10,
            0},{10,20}})));
  Buildings.Fluid.Sensors.TraceSubstances senCO2(redeclare package Medium =
        Medium) "Sensor at volume" annotation (extent=[14,20; 34,40], Placement(
        transformation(extent={{16,20},{36,40}})));
  Buildings.Fluid.Sources.TraceSubstancesFlowSource sou(
    redeclare package Medium = Medium,
    use_m_flow_in=true,
    nPorts=1) "CO2 source" annotation (extent=[-98,-70; -78,-50], Placement(
  ...
  Buildings.Fluid.MixingVolumes.MixingVolume ple(
    ...
    annotation (extent=[-10,-70; 10,-50], Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={-10,-60})));
