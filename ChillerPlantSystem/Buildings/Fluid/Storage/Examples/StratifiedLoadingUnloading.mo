within ChillerPlantSystem.Buildings.Fluid.Storage.Examples;
model StratifiedLoadingUnloading "Test model for stratified tank"
 ...
 parameter Modelica.SIunits.MassFlowRate m_flow_nominal= 1*1000/3600/4;
  ...
  Buildings.Fluid.Sources.Boundary_pT sou_1(
    ...
    use_T_in=false,
    nPorts=2) annotation (Placement(transformation(extent={{-100,-20},{-80,0}},
          rotation=0)));
  ...
  Buildings.Fluid.Storage.StratifiedEnhanced tanEnh(
    ...
    nSeg=nSeg,
    show_T=true,
    m_flow_nominal=m_flow_nominal) "Tank" annotation (Placement(transformation(
          extent={{-30,-2},{-10,18}}, rotation=0)));
  ...
  Buildings.Fluid.Sensors.EnthalpyFlowRate HIn_flow(redeclare package Medium =
        Medium, m_flow_nominal=m_flow_nominal) "Enthalpy flow rate" annotation (
     Placement(transformation(extent={{-60,-38},{-44,-22}}, rotation=0)));
  Buildings.Fluid.Sensors.EnthalpyFlowRate HOut_flow(redeclare package Medium
      = Medium, m_flow_nominal=m_flow_nominal) "Enthalpy flow rate" annotation (
     Placement(transformation(extent={{22,-38},{38,-22}}, rotation=0)));
  Buildings.Fluid.Sensors.EnthalpyFlowRate HInEnh_flow(redeclare package Medium
      = Medium, m_flow_nominal=m_flow_nominal) "Enthalpy flow rate" annotation (
     Placement(transformation(extent={{-60,0},{-44,16}}, rotation=0)));
  Buildings.Fluid.Sensors.EnthalpyFlowRate HOutEnh_flow(redeclare package
    ...
    annotation (Placement(transformation(extent={{2,0},{18,16}}, rotation=0)));
