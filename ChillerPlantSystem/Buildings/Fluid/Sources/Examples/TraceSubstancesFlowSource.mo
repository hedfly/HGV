within ChillerPlantSystem.Buildings.Fluid.Sources.Examples;
model TraceSubstancesFlowSource
  ...
  Buildings.Utilities.Diagnostics.AssertEquality assEqu(threShold=1E-4)
    "Assert that both volumes have the same concentration" annotation (
      Placement(transformation(extent={{210,128},{230,148}}, rotation=0)));
  ...
  Buildings.Utilities.Diagnostics.AssertEquality assEqu1(threShold=1E-4)
    "Assert that both volumes have the same concentration" annotation (
      Placement(transformation(extent={{210,0},{230,20}}, rotation=0)));
  ...
  Buildings.Fluid.Sources.Boundary_pT sin(
    ...
    nPorts=2,
    p=101320,
    T=293.15) "Sink boundary conditions" annotation (Placement(transformation(
          extent={{188,-50},{168,-30}}, rotation=0)));
