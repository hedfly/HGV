...
initial equation
  ...
  rho_nominal = Medium.density(
        Medium.setState_pTX(
          ...
          T=  0.5*(TSur_nominal+TFlu_nominal),
          X=  Medium.X_default));
  Pr_nominal = Buildings.Fluid.HeatExchangers.BaseClasses.prandtlNumberWater(T=
...
equation
  // Fluid properties
  mu = Buildings.Fluid.HeatExchangers.BaseClasses.dynamicViscosityWater(T=0.5*(
    TSur + TFlu));
  rho = Medium.density(
        Medium.setState_pTX(
          p=  Medium.p_default,
          T=  0.5*(TSur+TFlu),
          X=  Medium.X_default));
  Pr = Buildings.Fluid.HeatExchangers.BaseClasses.prandtlNumberWater(T=0.5*(
    TSur + TFlu));
