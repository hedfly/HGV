within ChillerPlantSystem.Buildings.Fluid.HeatExchangers.DXCoils.BaseClasses;
model Evaporation
...
protected
  ...
  constant Modelica.SIunits.SpecificHeatCapacity cpAir_nominal=Buildings.Media.PerfectGases.Common.SingleGasData.Air.cp
    "Specific heat capacity of air";
  constant Modelica.SIunits.SpecificHeatCapacity cpSte_nominal=Buildings.Media.PerfectGases.Common.SingleGasData.H2O.cp
...
initial equation
  ...
  if (dX_nominal > 1E-10) then
    Modelica.Utilities.Streams.print("Warning: In DX coil model, dX_nominal = "
       + String(dX_nominal) + "
  ...
  if computeReevaporation then
    ...
    if on then
      ...
      mEva_flow = -smooth(1, noEvent(dX*
        ...
        pos=if abs(mAir_flow) > mAir_flow_small/3 then abs(mAir_flow)*(1 -
          Modelica.Math.exp(-K2*m*abs(mAir_flow)^(-0.2))) else 0,
