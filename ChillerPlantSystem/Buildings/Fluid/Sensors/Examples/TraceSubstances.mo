within ChillerPlantSystem.Buildings.Fluid.Sensors.Examples;
model TraceSubstances "Test model for the extra property sensor"
 ...
 package Medium = Buildings.Media.GasesPTDecoupled.SimpleAir (
        extraPropertiesNames={"CO2"}) "Medium model";
 ...
 parameter Modelica.SIunits.MassFlowRate m_flow_nominal = 15*1.2/3600
  ...
  Buildings.Fluid.Sensors.TraceSubstances senVol(redeclare package Medium =
        Medium) "Sensor at volume" annotation (Placement(transformation(extent=
            {{100,50},{120,70}}, rotation=0)));
  Buildings.Fluid.Sensors.TraceSubstances senSou(redeclare package Medium =
        Medium, substanceName="CO2") "Sensor at source" annotation (Placement(
        transformation(extent={{24,90},{44,110}}, rotation=0)));
  ...
  Buildings.Fluid.Sources.MassFlowSource_T mSou(
    redeclare package Medium = Medium,
    use_m_flow_in=true,
    nPorts=1) "Fresh air supply" annotation (Placement(transformation(extent={{
            0,-22},{20,-2}}, rotation=0)));
  ...
  Buildings.Fluid.Sources.MassFlowSource_T mSin(
    redeclare package Medium = Medium,
    use_m_flow_in=true,
    nPorts=1) "Exhaust air" annotation (Placement(transformation(extent={{0,-62},
            {20,-42}}, rotation=0)));
  Buildings.Fluid.Sensors.Conversions.To_VolumeFraction masFraSou(MMMea=
        Modelica.Media.IdealGases.Common.SingleGasesData.CO2.MM)
    "Conversion of mass ratio to volume ratio" annotation (Placement(
        transformation(extent={{140,90},{160,110}}, rotation=0)));
  Buildings.Fluid.Sensors.Conversions.To_VolumeFraction masFraVol(MMMea=
        Modelica.Media.IdealGases.Common.SingleGasesData.CO2.MM)
    "Conversion of mass ratio to volume ratio" annotation (Placement(
        transformation(extent={{140,50},{160,70}}, rotation=0)));
  Buildings.Fluid.Sensors.TraceSubstancesTwoPort senTraSub(redeclare package
      Medium = Medium, m_flow_nominal=m_flow_nominal) "Sensor at exhaust air"
