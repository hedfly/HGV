...
algorithm
  ...
  incAngMod := Buildings.Utilities.Math.Functions.smoothHeaviside(Modelica.Constants.pi
    /3 - incAng, delta)*(1 + B0*(1/Buildings.Utilities.Math.Functions.smoothMax(
    Modelica.Math.cos(incAng),
    Modelica.Math.cos(incAngMin),
    delta) - 1) + B1*(1/Buildings.Utilities.Math.Functions.smoothMax(
    Modelica.Math.cos(incAng),
    cosIncAngMin,
    delta) - 1)^2);
