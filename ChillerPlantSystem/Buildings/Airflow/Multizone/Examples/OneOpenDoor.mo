...
model OneOpenDoor "Model with one open and one closed door"
  ...
  Buildings.Fluid.MixingVolumes.MixingVolume volA(
    ...
    nPorts=4,
    m_flow_nominal=0.01) annotation (Placement(transformation(extent={{-32,14},
            {-12,34}}, rotation=0)));
  Buildings.Fluid.MixingVolumes.MixingVolume volB(
    ...
    massDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial,
    nPorts=4,
    m_flow_nominal=0.01) annotation (Placement(transformation(extent={{60,60},{
            80,80}}, rotation=0)));
  Buildings.HeatTransfer.Sources.PrescribedHeatFlow preHeaFlo annotation (
      Placement(transformation(extent={{14,60},{34,80}}, rotation=0)));
